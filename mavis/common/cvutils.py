...
def color_coded_to_labelme(img_path,
...
class CVProcessor:
    def __init__(self):
        self.name = self.not_implemented
        self.i = self.not_implemented
        self.opts = self.not_implemented
    ...
    def configure_opt(self):
        ...
        self.opt = self.opts[st.selectbox("Type", list(self.opts.keys()), key=f"{self.name}{self.i}type")]()
...
class BlurProcessor(CVProcessor):
    ...
    def gaussian_blur_config(self):
        self.g_blur = st.number_input("Gaussian Blur Radius", 1, 15, (self.g_blur + 1) // 2,
                                      key=f"gblur{self.i}") * 2 - 1
        return self.gaussian_blur

    def median_blur_config(self):
        self.m_blur = st.number_input("Median Blur Radius", 1, 15, (self.m_blur + 1) // 2, key=f"mblur{self.i}") * 2 - 1
...
class MorphProcessor(CVProcessor):
    class Morph:
        ...
        def __init__(self, i):
            with st.beta_expander(f"Morph Operation {i + 1}"):
                self.kernel_size = st.slider("Kernel Radius", 0, 21, 3, key=f"kernel_size_{i}")
                self.structuring_element = st.selectbox("Structuring Element",
                                                        list(MorphProcessor.Morph.structuring_element_cv_types.keys()),
                                                        key=f"structuring_element_{i}")
                self.iterations = st.slider("Iteration", 0, 10, 1,
                                            key=f"iterations_{i}")
                self.morph_type = st.selectbox("Morph Type", list(MorphProcessor.Morph.morph_type_cv.keys()),
                                               key=f"morph_tpye_{i}")

        def run(self, img):
            structuring_cv_type = MorphProcessor.Morph.structuring_element_cv_types[self.structuring_element]
            element = cv.getStructuringElement(structuring_cv_type,
    ...
    def configure_opt(self):
        ...
        self.morph_operations = [MorphProcessor.Morph(i)
                                 for i in range(st.slider("Number of Morph Operations", 0, 5, 0))]
        pass
...
class ThresholdProcessor(CVProcessor):
    ...
    def binary_config(self):
        self.threshold = st.slider(f"Threshold limit", 0, 255, self.threshold, key=f"binathresh{self.i}")
        return self.binary

    def adaptive_gaussian_config(self):
        self.block_size = st.number_input("Block Radius", 0, 400, (self.block_size + 1) // 2,
                                          key=f"adapthresh{self.i}") * 2 + 1,
...
class EdgeProcessor(CVProcessor):
    ...
    def lap_config(self):
        self.ksize = st.number_input("Kernel Radius", 0, 255, (self.ksize + 1) // 2, key=f"lapksiz{self.i}") * 2 - 1
        return self.laplacian

    def canny_config(self):
        self.lower_threshold = st.slider("Lower Threshold", 0, 255, self.lower_threshold, key=f"canlt{self.i}"),
        self.upper_threshold = st.slider("Upper Threshold", 0, 255, self.upper_threshold, key=f"canut{self.i}")
        return self.canny
    ...
    def laplacian(self, gray):
        gray = -cv2.Laplacian(gray, cv2.CV_64F, ksize=self.ksize)
